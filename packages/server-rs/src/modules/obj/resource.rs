//! OBJ Resource routes: thin HTTP handlers delegating to services::obj.
...
pub use obj_svc::{ObjResOutput, ObjResRow, upsert_obj_resource};
...
async fn list(
    ...
    Ok(Json(obj_svc::list_resources(&state.db.pool, game_id).await?))
}
...
async fn get(
    ...
    Path(id): Path<Uuid>,
    Query(q): Query<GameQuery>,
) -> ApiResult<Json<obj_svc::ObjResOutput>> {
    let game_id = verify_game_access(&state, &q.game_id, auth.0).await?;
    Ok(Json(obj_svc::get_resource(&state.db.pool, game_id, id).await?))
}
...
async fn create(
    ...
    Ok(Json(obj_svc::create_resource(&state.db.pool, game_id, &input.key, &input.data).await?))
}
...
async fn update(
    ...
    let game_id = verify_game_access(&state, &input.game_id, auth.0).await?;
    Ok(Json(obj_svc::update_resource(&state.db.pool, game_id, id, &input.data).await?))
}
...
async fn delete(
    ...
    let game_id = verify_game_access(&state, &q.game_id, auth.0).await?;
    let id = obj_svc::delete_resource(&state.db.pool, game_id, id).await?;
